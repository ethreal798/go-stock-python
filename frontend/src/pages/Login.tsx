...
const Login: React.FC = () => {
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', overflow: 'hidden' }}>
      {/* 左侧 70% 宽度的大图片区域 */}
      <Col span={17} style={{ background: '#001529', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff' }}>
        <div style={{ textAlign: 'center' }}>
          <Title style={{ color: '#fff', fontSize: '48px' }}>Go-Stock</Title>
          <Text style={{ color: 'rgba(255,255,255,0.65)', fontSize: '18px' }}>专业的股票分析与量化交易平台</Text>
          {/* 这里留出图片位置，目前用渐变色代替背景 */}
          <div style={{ 
            marginTop: '40px', 
            width: '80%', 
      ...
      {/* 右侧 30% 登录注册表单 */}
      <Col span={7} style={{ background: '#fff', padding: '0 40px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        <div style={{ maxWidth: '400px', width: '100%', margin: '0 auto' }}>
          <div style={{ marginBottom: '40px', textAlign: 'center' }}>
            <Title level={2}>{isRegister ? '创建账号' : '欢迎回来'}</Title>
            <Text type="secondary">
              {isRegister ? '请输入您的基本信息开始体验' : '请输入您的邮箱和密码进行登录'}
            </Text>
          </div>
          ...
          <Divider plain><Text type="secondary" style={{ fontSize: '12px' }}>其他操作</Text></Divider>
