...
const Settings: React.FC = () => {
  const { settings, updateAI, updateNotify, updateDataSource, setLoading, loading, setSaved } =
    useSettingsStore()
  ...
  const handleSaveAI = async () => {
    try {
      ...
      setLoading(true)
      await request.put('/settings/ai', values)
      updateAI(values)
  ...
  const tabItems = [
    {
      ...
      children: (
        <Form form={aiForm} layout="vertical" style={{ maxWidth: 600 }}>
          ...
          <Form.Item>
            <Space>
              <Button type="primary" icon={<SaveOutlined />} onClick={handleSaveAI} loading={loading}>
                保存配置
              </Button>
              <Button icon={<UndoOutlined />} onClick={() => aiForm.setFieldsValue(settings.ai)}>
                重置
              </Button>
            </Space>
          </Form.Item>
    ...
    },
    {
      ...
      children: (
        <Form form={notifyForm} layout="vertical" style={{ maxWidth: 600 }}>
          ...
          <Form.Item>
            <Button type="primary" icon={<SaveOutlined />} onClick={handleSaveNotify} loading={loading}>
              保存配置
            </Button>
          </Form.Item>
        </Form>
      ...
      children: (
        <Form form={dsForm} layout="vertical" style={{ maxWidth: 600 }}>
          <Form.Item
            label="Tushare Token"
            name="tushareToken"
            extra={<Text type="secondary">前往 <a href="https://tushare.pro" target="_blank" rel="noreferrer">tushare.pro</a> 获取 Token</Text>}
          >
          ...
          <Form.Item>
            <Button type="primary" icon={<SaveOutlined />} onClick={handleSaveDS} loading={loading}>
              保存配置
            </Button>
          </Form.Item>
        </Form>
