...
import { getStreamChatUrl, getSessionList, createSession, clearSession } from '@/api/agent'
...
const Agent: React.FC = () => {
  ...
  const { loading: sseLoading, connect: sseConnect, abort: sseAbort } = useSSE({
    onMessage: (data) => {
      try {
    ...
    onError: () => {
      setMessages((prev) =>
        prev.map((m) =>
          m.loading ? { ...m, loading: false, error: true, content: '请求失败，请重试' } : m,
        ),
      )
    },
  })
    ...
    <div style={{ display: 'flex', height: 'calc(100vh - 80px)', gap: 12 }}>
      ...
      >
        <List
          ...
          renderItem={(session) => (
            <List.Item
              onClick={() => { setCurrentSession(session); setMessages(session.messages ?? []) }}
              style={{
                cursor: 'pointer',
                padding: '8px 12px',
            ...
            >
              <Text ellipsis style={{ width: '100%' }}>{session.title || '新会话'}</Text>
            </List.Item>
          )}
        />
      </Card>
      ...
      >
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '12px 16px' }}>
          ...
          {messages.map((msg) => (
            ...
            >
              {msg.role === 'assistant' && (
                <Avatar icon={<RobotOutlined />} style={{ background: '#1677ff', flexShrink: 0, marginRight: 8 }} />
              )}
              <div
                style={{
                  maxWidth: '72%',
              ...
              {msg.role === 'user' && (
                <Avatar icon={<UserOutlined />} style={{ background: '#87d068', flexShrink: 0, marginLeft: 8 }} />
              )}
            </div>
          ))}
