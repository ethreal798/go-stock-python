...
const CronTasks: React.FC = () => {
  ...
  const statusMap: Record<string, { status: 'processing' | 'default' | 'error'; text: string }> = {
  ...
  const columns: ColumnsType<CronTask> = [
    ...
    { title: 'Cron表达式', dataIndex: 'cronExpr', width: 140, render: (v: string) => <code style={{ background: '#f5f5f5', padding: '2px 6px', borderRadius: 3 }}>{v}</code> },
    { title: '任务类型', dataIndex: 'taskType', render: (v: string) => <Tag color="blue">{v}</Tag> },
    {
      title: '状态',
      dataIndex: 'status',
      width: 80,
    ...
    >
      ...
      >
        <Form form={form} layout="vertical">
          ...
          <Form.Item label="Cron表达式" name="cronExpr" rules={[{ required: true, message: '请输入Cron表达式' }]}>
            <Input placeholder="如: 0 9 * * 1-5 （工作日9点）" />
          </Form.Item>
          <Form.Item label="任务类型" name="taskType" rules={[{ required: true, message: '请输入任务类型' }]}>
