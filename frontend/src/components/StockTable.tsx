...
import { DeleteOutlined, LineChartOutlined, ArrowUpOutlined, ArrowDownOutlined } from '@ant-design/icons'
...
const ChangeCell: React.FC<{ value: number; suffix?: string }> = ({ value, suffix = '%' }) => {
  ...
  const columns: ColumnsType<Stock> = [
    {
      ...
      render: (code: string, record) => (
        <Space size={4}>
          ...
          {record.market && <Tag style={{ fontSize: 10, padding: '0 3px', lineHeight: '16px' }}>{record.market}</Tag>}
        </Space>
      ),
    },
    {
    ...
    },
    {
      ...
      key: 'changeRate',
      width: 90,
      sorter: (a, b) => (prices[a.code]?.changeRate ?? 0) - (prices[b.code]?.changeRate ?? 0),
      render: (_, record) => {
        const p = prices[record.code]
    ...
    },
    {
      ...
      render: (_, record) => {
        const p = prices[record.code]
        return p ? <span style={{ color: '#f5222d' }}>{formatNumber(p.high)}</span> : '-'
      },
    },
    {
      title: '最低',
      ...
      render: (_, record) => {
        const p = prices[record.code]
        return p ? <span style={{ color: '#52c41a' }}>{formatNumber(p.low)}</span> : '-'
      },
    },
    {
      title: '换手率',
