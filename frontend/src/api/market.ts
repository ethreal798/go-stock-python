...
// 获取异动股
export const getAbnormalStocks = () =>
  request.get('/market/abnormal')

// 获取行业板块行情
export const getSectorData = (params?: { type?: 'industry' | 'concept' | 'area' }) =>
  request.get('/market/sectors', { params })

// 获取市场资金流向
export const getMarketMoneyFlow = () =>
  request.get('/market/money-flow')
...
export const getNewsList = (params?: {
  ...
  pageSize?: number
  keyword?: string
}) => request.get<{ data: NewsItem[] }>('/market/news', { params })

// 获取快讯/电报
export const getFlashNews = (params?: { page?: number; pageSize?: number }) =>
  request.get<{ data: NewsItem[] }>('/market/flash-news', { params })

// 获取全球市场数据
export const getGlobalMarket = () =>
  request.get('/market/global')

// 获取A股市场统计
export const getMarketStatistic = () =>
  request.get('/market/statistic')
