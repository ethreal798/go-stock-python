...
def upgrade() -> None:
    ...
    op.execute(
        "ALTER TABLE fund_watchlist_items "
        "RENAME CONSTRAINT followed_funds_pkey TO fund_watchlist_items_pkey"
    )
...
def downgrade() -> None:
    ...
    op.execute(
        "ALTER TABLE fund_watchlist_items "
        "RENAME CONSTRAINT fund_watchlist_items_pkey TO followed_funds_pkey"
    )
